...
// Check support for Open multi processing and enable it
#ifdef _OPENMP
#include <omp.h>
#endif
...
void process(const vector<string> &argv,
    ...
    */
    ...
    printf("# quantitative values after filtering = %d\n\n", n_after_filtered);
...
py::list iq_filter(py::list cmd) {

    // const char *c = CHAR(STRING_ELT(cmd, 0));  
    // this line expects a frist element of cmd is a string for R -> c
    const char *c = cmd[0].cast<std::string>().c_str();
    ...
    /*
    Create p_index create matrix of size (protein_index->size()-row x 1-col) type INTSXP
    AND copy elements from protein_index->begin() to protein_index->end() to p_index

    convert SEXP to py::array_t<int> p_index
    p_index : type py::array_t
    ...
    */ 
    py::array_t<int> p_index(protein_index->size(),protein_index->data());

    py::array_t<int> s_index(sample_index->size(),sample_index->data());

    py::array_t<int> i_index(ion_index->size(),ion_index->data());

    py::array_t<double> i_index(ion_index->size(),ion_index->data());
    ...
    py::array_t<std::string> ion_str(ions->size());
    for (size_t i = 0; i < ions->size(); i++) {
        // concatenate
        ion_str.mutable_at(i) = utils::concatenate(*(*ions)[i]);
    }
    ...
    py::array_t<std::string> sample_str(samples->size());
    for (size_t i = 0; i < samples->size(); i++) {
        sample_str.mutable_at(i) = *(*samples)[i];
    }
    ...
    py::array_t<std::string> ann({annotation->size(), (*annotation)[0]->size()});
    for (size_t i = 0; i < annotation->size(); i++) {
        for (size_t j = 0; j < (*annotation)[i]->size(); j++) {
            ann.mutable_at(i, j) = (*annotation)[i]->at(j);
        }
    }
    ...
    py::array_t<std::string> col_names(col_annotation->size());
    for (size_t i = 0; i < col_annotation->size(); i++) {
        col_names.mutable_at(i) = col_annotation->at(i);
    }
    ...
    py::list dimnames(2);
    dimnames[1] = col_names;
    ann.attr("dimnames") = dimnames;
    ...
    py::list quant_tab_names = py::cast(std::vector<std::string>{"protein_list", "sample_list", "id", "quant"});
    quant_tab.attr("__name__") = quant_tab_names;
    ...
    py::list names = py::cast(std::vector<std::string>{"quant_table", "protein", "sample", "ion"});
    vec.attr("__name__") = names;
...
PYBIND11_MODULE(example, m) {
    m.def("iq_filter", &iq_filter, "iq_filter");
}
...
void tp_user(void *dummy) {
    if (std::signal(SIGINT, SIG_IGN) != SIG_IGN) {
...
py::list iq_MaxLFQ(py::dict list) {
    ...
    size_t nrow = get_list_element(list, "protein_index").cast<py::list>().size();
    ...
    py::array_t<double> table({n_proteins, n_samples});
    double* buffer = static_cast<double*>(table.request().ptr);
    ...
    printf("nrow = %d, # proteins = %d, # samples = %d\n", nrow, n_proteins, n_samples);
    ...
    for (int i = 0; i < n_proteins; i++) {
        r_names[i] = std::to_string(row_names[i]);
    }
    ...
    for (int i = 0; i < n_samples; i++) {
        c_names[i] = std::to_string(col_names[i]);
    }
...
PYBIND11_MODULE(example, m) {
    m.def("iq_MaxLFQ", &iq_MaxLFQ, "iq_MaxLFQ");
}
